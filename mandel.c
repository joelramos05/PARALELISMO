...
int main (int argc, char **argv)
{
  ...
  gettimeofday(&ti, NULL);
  MPI_Gather(local_data, filasProcs * X_RESN, MPI_INT, vres, filasProcs * X_RESN, MPI_INT, 0, MPI_COMM_WORLD);
  gettimeofday(&tf, NULL);
  ...
  if(myRank == 0) {
    for (i = 0; i < numProcs; i++) {
      fprintf(stderr, "Proc %2d: Computacion %8.6f Comunicacion %8.6f\n", i, comp[i], comm[i]);
    }
    /* Print result out */
    if( DEBUG ) {
      for(i=0;i<Y_RESN;i++) {
...
set size square
set palette grey
plot 'salida.txt' matrix with image
*/
